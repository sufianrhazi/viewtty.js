...
var Player = class {
  ...
  _step() {
    if (this._lastTick === null) {
      ...
      if (this._frame < this._chunks.length) {
        this._lastTick = this._chunks[this._frame].ms;
      }
    }
    var now = this.now();
    var chunks = [];
    ...
    var startIndex;
    let i;
    for (i = this._frame; i < this._chunks.length && this._chunks[i].ms <= now; ++i) {
    ...
    if (this._frame < this._chunks.length) {
      ...
      var delta = this._chunks[this._frame].ms - now;
      console.log("NEXT", {
        chunk: this._chunks[this._frame],
        now,
        delta
      });
...
var Timer = class {
  constructor() {
    ...
    this.setTimeout = (f, ms) => {
      ...
      const id = this.id++;
      console.log("-> setTimeout", { now: this._now, when, ms, id });
  ...
  advance(ms) {
    ...
    for (let id = 0; id < this.id; ++id) {
      console.log("-> advance", { now: this._now, id });
      const event = this.events.get(id);
      if (event && !event.cleared && event.ms <= this._now) {
        this.events.delete(id);
        console.log("-> advance", { ms: event.ms, id }, "run");
        event.fn();
      }
    }
  }
...
if (false) {
  test("does not play until played", () => {
    ...
    import_assert.default.deepEqual(
      [
        {
          data: void 0,
          type: "rewind"
...
test("plays when played", () => {
  ...
  timer.advance(20);
  import_assert.default.deepEqual([], messages);
});
