...
Player.prototype._emit = function _emit(type, data) {
    this.listeners.forEach(function (f) {
        try {
            ...
            this._timer.setTimeout(function () { throw e; }, 0);
        }
    }.bind(this));
};
Player.prototype.pause = function pause() {
    if (this._tickHandle) {
        this._emit('pause');
        this._timer.clearTimeout(this._tickHandle);
        this._tickHandle = null;
    }
};
...
Player.prototype._step = function _step() {
    ...
    if (this._frame < this._chunks.length) {
        ...
        this._tickHandle = this._timer.setTimeout(this._step.bind(this), nextChunk.ms - currentChunk.ms);
    } else {
        this._emit('end');
    }
};
